...
help:
	...
	@echo "TOOLCHAIN = <gcc, clang> : set compiler toolchain (default is gcc)"
	@echo "LD = <ld, ld.lld, ...> : override linker executable (e.g. ld.lld)"
	@echo "LTO_MODE = <off, thin, full> : link-time optimization (default is off; thin/full require TOOLCHAIN=clang + ld.lld)"
	@echo "TOOLCHAIN_PREFIX = <absolute path to toolchain or relative path with prefix>"
	@echo "TOOLCHAIN_<arch>_PREFIX = <absolute path to toolchain or relative path with prefix for a particular architecture>"
	...
	@echo "make testproject TOOLCHAIN=clang"
	@echo "make testproject TOOLCHAIN=clang LD=ld.lld"
	@echo "make testproject TOOLCHAIN=clang LTO_MODE=thin"
