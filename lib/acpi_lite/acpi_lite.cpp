...
status_t acpi_lite_init(paddr_t rsdp_pa) {
    ...
    const acpi_rsdp *rsdp_ptr = nullptr;
    const void *rsdp_map = nullptr;
    bool rsdp_from_immed = false;
    bool rsdp_mapped = false;

    auto cleanup_rsdp_mapping = lk::make_auto_call([&]() {
        if (rsdp_mapped && rsdp_map) {
            vmm_free_region(vmm_get_kernel_aspace(), ROUNDDOWN((vaddr_t)rsdp_map, PAGE_SIZE));
            acpi.rsdp_pa = 0;
            acpi.rsdp = nullptr;
        }
    ...
    if (rsdp_pa == 0) {
        rsdp_ptr = static_cast<const acpi_rsdp *>(get_immed_rsdp());
        if (rsdp_ptr) {
            rsdp_from_immed = true;
        }
    ...
    if (!rsdp_from_immed) {
        ...
        rsdp_map = map_region(rsdp_pa, rsdp_area_len, "acpi rsdp area");
        rsdp_ptr = static_cast<const acpi_rsdp *>(rsdp_map);
        if (!rsdp_ptr) {
    ...
    acpi.rsdp = rsdp_ptr;
    acpi.rsdp_pa = rsdp_from_immed ? 0 : rsdp_pa;
