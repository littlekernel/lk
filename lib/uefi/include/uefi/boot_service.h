...
EFI_ENUM(EfiOpenProtocolAttributes, uint32_t,
         ...
         EFI_OPEN_PROTOCOL_ATTRIBUTE_BY_CHILD_CONTROLLER = 0x00000008,
         EFI_OPEN_PROTOCOL_ATTRIBUTE_BY_DRIVER = 0x00000010,
         EFI_OPEN_PROTOCOL_ATTRIBUTE_EXCLUSIVE = 0x00000020);
...
typedef struct {
  ...
  EfiStatus (*exit)(EfiHandle image_handle, EfiStatus exit_status,
                    size_t exit_data_size, EfiChar16* exit_data);
  ...
  EfiStatus (*set_watchdog_timer)(size_t timeout, uint64_t watchdog_code,
                                  size_t data_size, EfiChar16* watchdog_data);
