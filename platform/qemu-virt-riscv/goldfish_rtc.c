...
volatile uint32_t * const goldfish_rtc_base = (void *)RTC_BASE_VIRT;
...
lk_time_t rtc_current_time(void) {
    ...
    return (lk_time_t)(t / 1000000000); // ns -> ms
}
...
enum handler_return rtc_irq(void *unused) {
    ...
    if (t_callback) {
        ret = t_callback(t_arg, rtc_current_time());
    }

    return ret;
}

void rtc_platform_stop_timer(void) {
    LTRACE;
...
void goldfish_rtc_early_init(void) {
    ...
    rtc_platform_stop_timer();

    register_int_handler(RTC_BASE_VIRT, &rtc_irq, NULL);
    unmask_interrupt(RTC_BASE_VIRT);
...
void goldfish_rtc_init(void) {
	TRACEF("starting Goldfish RTC device\n");
}
